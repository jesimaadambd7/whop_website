...
export function middleware(request: NextRequest) {
  ...
  const hasAdminSession = Boolean(request.cookies.get("vidcarry_admin_session")?.value);
  ...
  const hasCreatorSession = Boolean(request.cookies.get("vidcarry_creator_session")?.value);
